// History Component - Lazy Loaded
import React, { useState, useEffect } from 'react';
import { ProgressiveLoader } from '../../utils/lazyLoading';

const HistoryComponent = ({ onQuestionComplete, gameSettings = {} }) => {
  ...
  const loadHistoryContent = async () => {
    try {
      ...
      // Dynamically import history content
      const { historyQuestions, historyAchievements } = await import('../../data/subjects/historyContent');
      ...
      setAchievements(historyAchievements);
      
      // Track loading performance
      if (window.emmyPerformance?.monitor) {
        window.emmyPerformance.monitor.recordCustomMetric('history_content_loaded', performance.now());
      }
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-amber-400 via-orange-500 to-red-600">
        <div className="text-center text-white">
          <div className="text-6xl mb-4">🏛️</div>
          <div className="text-2xl mb-4">Loading History Adventure...</div>
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-amber-400 via-orange-500 to-red-600">
        <div className="text-center text-white">
          ...
          <div className="text-2xl mb-4">Failed to load History content</div>
          <button 
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-amber-400 via-orange-500 to-red-600 p-4">
      <div className="max-w-4xl mx-auto">
        {/* Header */}
        <div className="text-center mb-8">
          <div className="text-6xl mb-4">{question.image || '🏛️'}</div>
          <h1 className="text-4xl font-bold text-white mb-2">History Adventure</h1>
